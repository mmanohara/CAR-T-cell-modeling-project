...
def initialize():
    ...
    for y in range(n):#find the total cancer population at each point
        for x in range(n):
            ctotal[x, y] = c0[x, y] + c1[x, y] + c2[x, y] + c3[x, y] + cco[x, y]
    t1 = q * exp(-((xvalues - .5)**2 + (yvalues - .5)**2) / (.05**2))
    nextt1 = zeros([n, n])#define variables of next timestep as an array
    nextt2 = zeros([n, n])
    nextt3 = zeros([n, n])
    nextc0 = zeros([n, n])
    nextc1 = zeros([n, n])
    ...
    ttotal = 0#set total populations as zero
    c0total = 0
    c1total = 0
    c2total = 0
    c3total = 0
    ...
    for x in range(100):#find the max value to use for the colorbar
        for y in range(100):
            ...
            if c0plot[x, y] > c0max:
                c0max = c0plot[x, y]
                c0xcenter = x#these are the x and y points where the max population is
                c0ycenter = y
            if c1plot[x, y] > c1max:
                c1max = c1plot[x, y]
                c1xcenter = x
...
def observe():
    ...
    colorbar()
    subplot(5, 5, 5)
    imshow(c2plot, vmin = 0, vmax = c2max)#showEGFR
    title('EGFR')
    colorbar()
    subplot(5, 5, 11)
    imshow(c3plot, vmin = 0, vmax = c3max)#showIL13ra2
    title('IL13ra2')
    colorbar()
    subplot(5, 5, 21)
    ...
    title('coexpression')
    colorbar()
    subplot(5, 5, 23)#plot the total population sizes vs time
...
def update():
    global t1, t2, t3, c0, c1, c2, c3, cco, ctotal, nextt1, nextt2, nextt3, nextc0, nextc1, nextc2, nextc3, nextcco, nextctotal, ttotal, c0total, c1total, c2total, c3total, ccototal, tplot, c0plot, c1plot, c2plot, c3plot, ccoplot, step, check, tmax, c0max, c1max, c2max, c3max, ccomax, tpend, c0pend, c1pend, c2pend, c3pend, ccopend, steppend, c0xcenter, c0ycenter, c1xcenter, c1ycenter, c2xcenter, c2ycenter, c3xcenter, c3ycenter, ccoxcenter, ccoycenter
    if step<7.00001 and step > 6.99999:#at every 7 time steps, add a different kind of T-cells to the area of highest concentration
        t2 = t2 + 0.5 * q * exp(-((xvalues - c2xcenter)**2 + (yvalues - c2ycenter)**2) / (.05**2)) + 0.5 * q * exp(-((xvalues - c3xcenter)**2 + (yvalues - c3ycenter)**2) / (.05**2))         
    if step<14.00001 and step > 13.99999:
        t3 = t3 + 0.5 * q * exp(-((xvalues - c1xcenter)**2 + (yvalues - c1ycenter)**2) / (.05**2)) + 0.5 * q * exp(-((xvalues - c2xcenter)**2 + (yvalues - c2ycenter)**2) / (.05**2))         
    if step<21.00001 and step > 20.99999:
        t1 = t1 + 0.5 * q * exp(-((xvalues - c1xcenter)**2 + (yvalues - c1ycenter)**2) / (.05**2)) + 0.5 * q * exp(-((xvalues - c3xcenter)**2 + (yvalues - c3ycenter)**2) / (.05**2))         
    for x in range(1, n-1):
	    ...
	    """
            ...
            nextc1[x, y] = c1C + (a * c1C * (1 - (ctotalC)/K) - b1 * c1C * (t1C + t3C) + Dc * (c1LapNum / dh**2)) * dt
    ...
    ttotal = 0#reset the total population size
    c0total = 0
    c1total = 0
    c2total = 0
    c3total = 0    
    ...
    for x in range(1, n-1):#sum up the total each cell type.
        for y in range(1, n-1):
            ...
            ccototal = ccototal + cco[x, y]
    for x in range(1, n-1):
        for y in range(1, n-1):#update plotted variables with new values
            tplot[x-1, y-1] = t1[x, y] + t2[x, y] + t3[x, y]
            c0plot[x-1, y-1] = c0[x, y]
            c1plot[x-1, y-1] = c1[x, y]
            c2plot[x-1, y-1] = c2[x, y]
...
def takepic():#defining when to take the pic, and what the pic is saved as.
    global t1, t2, t3, c0, c1, c2, c3, cco, ctotal, nextt1, nextt2, nextt3, nextc0, nextc1, nextc2, nextc3, nextcco, nextctotal, ttotal, c0total, c1total, c2total, c3total, ccototal, tplot, c0plot, c1plot, c2plot, c3plot, ccoplot, step, check, tmax, c0max, c1max, c2max, c3max, ccomax, tpend, c0pend, c1pend, c2pend, c3pend, ccopend, steppend, c0xcenter, c0ycenter, c1xcenter, c1ycenter, c2xcenter, c2ycenter, c3xcenter, c3ycenter, ccoxcenter, ccoycenter
    if check == 50:#pic every 50 time steps
        observe()
        savefig(str(step) + 'optimal.png')
        check = 0
...
def picmode():#the mode when we want to save a bunch of figures
    ...
    while step < 28:
        update()
        takepic()
    figtext(0.8, 0.4, 'T =' + str(ttotal))#after 28 time, show the final population sizes
